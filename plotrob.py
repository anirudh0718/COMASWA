...
def plot_step(state_hist, plot_handle,new_obs,centre,angle,gridlength,goal,id,time):
    ...
    plot_handle.set_xlim([-2.5, 1])
    plot_handle.set_ylim([-1, 1.3])
    plot_handle.plot(goal[0], goal[1], color='k', marker='X',markersize=10)
    ...
    if id == 1:
        plot_handle.plot(state_hist_plot[-1, 0], state_hist_plot[-1, 1], marker='h',markersize=15) # current
    else:
        plot_handle.plot(state_hist_plot[-1, 0], state_hist_plot[-1, 1], marker=(8, 0),markersize =15) # current
    ...
    #print((centre_hist))
    ell3 = Ellipse((centre_hist[-1, 0], centre_hist[-1, 1]), 0.5, 0.5, angle_hist[-1])
